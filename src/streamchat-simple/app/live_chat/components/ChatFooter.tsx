...
const ChatFooter = ({ csrfToken, user }: Props) => {
  ...
  return (
    <footer className="h-[111px] flex-none mt-auto p-4 bg-white dark:bg-black border-gray-300 dark:border-gray-700 border-t">
      <div className="ml-2 flex">
        <CurrentUserAvatar uid={user.uid} />
        <div className="flex-row px-4 w-full justify-start">
          <div className="text-[13px] text-black dark:text-white text-opacity-50">
            {user.displayName}
          </div>
          <form className="w-full" onSubmit={handleSubmit(csrfToken)}>
            <input
              ...
              className="appearance-none bg-transparent placeholder-black dark:placeholder-white
                  placeholder-opacity-70 border-none w-full text-[13px] text-black dark:text-white leading-tight focus:outline-none"
            />

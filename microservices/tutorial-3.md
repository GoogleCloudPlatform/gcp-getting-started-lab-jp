...
> Cloud Shellの端末に表示されたリンクをクリックすると、端末画面の右端でリンクの文字列が切れるため「認証エラー（エラー 400: invalid_request / Required parameter is missing: response_type）」と表示されることがあります。そのような場合は、リンクをそのままクリックするのではなく、コピー＆ペーストでブラウザに入力してください。

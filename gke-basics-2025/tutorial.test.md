...
`プロジェクト ID` は [ダッシュボード](https://console.cloud.google.com/home/dashboard) に進み、左上の **プロジェクト情報** から確認可能です。
...
```bash
gcloud services enable compute.googleapis.com container.googleapis.com artifactregistry.googleapis.com cloudbuild.googleapis.com iam.googleapis.com iamcredentials.googleapis.com storage.googleapis.com gkehub.googleapis.com
```
...
GKE を利用するには、VPC 、サブネット、NAT などの設定が必要になりますが、今回はクラスタ及び一度に必要なものを作成できるように Terraform テンプレートを利用します。 

GKE 以下のコマンドを実行し、GKE Autopilot クラスタを作成します。
Terraform の初期化と適用 Terraform を使用するために、まず作業ディレクトリを初期化します。

```bash
terraform init
```

次に、project_id 変数に環境変数の値を渡して、Terraform の実行計画を確認し、適用します。 
...
```
インフラの作成には、20〜30分程度の時間がかかる場合があります。完了後、出力値が表示されます。

## **Lab01.サンプルアプリケーションのデプロイ**

<walkthrough-tutorial-duration duration=20></walkthrough-tutorial-duration>

クラスタの作成が完了しましたら、GKEクラスタに接続するための認証情報を取得します。また、環境変数にGKEクラスタ名、リージョン、GCSバケット名などを設定しておきます。
```bash
export GKE_CLUSTER_NAME=$(terraform output -raw gke_cluster_name)
export REGION=$(terraform output -raw region_output)
export APP_KSA_NAME=$(terraform output -raw app_ksa_name_output)
export APP_NAMESPACE=$(terraform output -raw app_namespace_output)
export DEMO_BUCKET_NAME=$(terraform output -raw demo_bucket_name)
export DEMO_OBJECT_NAME=$(terraform output -raw demo_object_name)
export WI_POOL=$(terraform output -raw workload_identity_pool)
```

正しく設定されているか確認します。
```bash
echo "GKE Cluster: $GKE_CLUSTER_NAME in $REGION"
echo "App KSA: $APP_KSA_NAME in namespace $APP_NAMESPACE"
echo "Demo GCS Bucket: $DEMO_BUCKET_NAME, Object: $DEMO_OBJECT_NAME"
...
```
kubectl コマンドがクラスタと通信できることを確認します。

```bash
kubectl get nodes
```
このように kubernetes では専用の CLI である kubectl や API などで操作、ステータス、設定値の確認などが可能です。

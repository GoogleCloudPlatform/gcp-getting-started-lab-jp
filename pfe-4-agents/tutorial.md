...
> この v16 は「可読版」です。Python や JSON の中身は `assets/` ディレクトリに通常ファイルとして分離しています。
> Teachme の実行ボタンで壊れやすい heredoc は使わず、必要な箇所では `assets/` からコピーまたは実行します。
...
```bash
python "${HANDSON_ASSETS_DIR}/check_01.py"
```

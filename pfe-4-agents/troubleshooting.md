...
## 4.1. `Cannot install mcp` / `referencing` の依存関係エラーが出る

以下のようなエラーが出る場合は、Python のバージョンが古い、または `mcp` が未固定のまま最新依存関係を解決しようとしている可能性があります。`fastmcp==2.13.1` と `mcp` は Python 3.10 以上を前提にしています。

```text
ERROR: Cannot install mcp because these package versions have conflicting dependencies.
jsonschema ... depends on referencing>=0.28.4
Additionally, some packages in these conflicts have no matching distributions available for your environment:
    referencing
```

このハンズオンでは Python 3.10 以上が必要です。仮想環境を作り直してから再インストールします。

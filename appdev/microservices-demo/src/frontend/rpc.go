...
func (fe *frontendServer) getAllCoupons(ctx context.Context, sessionID string) ([]*pb.Coupon, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Millisecond*3000)
	defer cancel()

	resp, err := pb.NewCouponServiceClient(fe.couponSvcConn).
		GetCoupons(ctx, &pb.CouponRequest{
			SessionId: sessionID,
		})
	return resp.GetCoupons(), err
}

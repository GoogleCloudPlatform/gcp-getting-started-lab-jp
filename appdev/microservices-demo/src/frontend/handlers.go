...
func (fe *frontendServer) homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	cp, err := fe.viewBestCoupon(r.Context(), sessionID(r), log)
	if err != nil {
		log.WithField("error", err).Error("cannot retrieve the best coupon")
	}
...
func (fe *frontendServer) viewBestCoupon(ctx context.Context, sessionID string, log logrus.FieldLogger) (*couponView, error) {
	coupons, err := fe.getAllCoupons(ctx, sessionID)
	if err != nil {
		log.WithField("error", err).Error("failed to retrieve coupons")
		return nil, err
	}

	if len(coupons) > 0 {
		sort.Slice(coupons, func(i, j int) bool { return coupons[i].DiscountPercentage < coupons[j].DiscountPercentage })

		t := time.Unix(coupons[len(coupons)-1].ExpiredBy, 0)
		expired_by := t.Format(time.RFC3339)

		return &couponView{coupons[len(coupons)-1].DiscountPercentage, expired_by}, nil

	} else {
		return &couponView{}, nil
	}
}

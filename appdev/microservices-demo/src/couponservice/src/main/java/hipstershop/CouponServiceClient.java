...
public class CouponServiceClient {
  ...
  private static final Logger logger = LogManager.getLogger(CouponServiceClient.class);
  private static final Tracer tracer = Tracing.getTracer();

  private final ManagedChannel channel;
  private final hipstershop.CouponServiceGrpc.CouponServiceBlockingStub blockingStub;

  /** Construct client connecting to Ad Service at {@code host:port}. */
  private CouponServiceClient(String host, int port) {
    this(
  ...
  private CouponServiceClient(ManagedChannel channel) {
    this.channel = channel;
    blockingStub = hipstershop.CouponServiceGrpc.newBlockingStub(channel);
  }
  ...
  /** Get Ads from Server. */
  public void getCoupons(String sessionId) {
    logger.info("Get Coupon with session_id " + sessionId +  " ...");
    CouponRequest request = 
    CouponRequest.newBuilder()
    ...
    Span span =
        tracer
            .spanBuilder("CouponClient")
            .setRecordEvents(true)
            .setSampler(Samplers.alwaysSample())
            .startSpan();
    try (Scope scope = tracer.withSpan(span)) {
      tracer.getCurrentSpan().addAnnotation("Getting Coupons");
      response = blockingStub.getCoupons(request);
      tracer.getCurrentSpan().addAnnotation("Received response from Coupons Service.");
    ...
    for (Coupon coupon : response.getCouponsList()) {
      logger.info("Discount: " + coupon.toString());
    }
  }
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    final String host = getStringOrDefaultFromArgs(args, 0, "localhost");
    final int serverPort = getPortOrDefaultFromArgs(args, 1, 9555);
    ...
    CouponServiceClient client = new CouponServiceClient(host, serverPort);
    try {
      String sessionId = "aaaaaaaa-1111-bbbb-2222-cccccccccccc";
      client.getCoupons(sessionId);
      sessionId = "33333333-dddd-4444-eeee-555555555555";
      client.getCoupons(sessionId);
    } finally {
      client.shutdown();
    }

    logger.info("Exiting CouponServiceClient...");
  }
}
